...
int CInteractive::L_RPCCall(lua_State *L)
{
    ...
    Object reply;
    if (!ptr->pRPCClient->CallRPC(lua_tostring(L,1),param.get_obj(),reply,strHost,nPort))
    {
...
int CInteractive::L_RPCJson(lua_State *L)
{
    ...
    Object reply;
    if (!ptr->pRPCClient->CallRPC(lua_tostring(L,1),valParam.get_obj(),reply,strHost,nPort))
    {
        return L_Error(L,-32603,"rpc failed");
    }
